...
# Written by Ze Liu
# --------------------------------------------------------
# This is the official implementation from SwinTransformer
...
def swin_tiny(pretrained=None):
    #model = SwinTransformer(img_size=224, num_classes=0,depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],embed_dim=96,drop_path_rate=0.1)
    
    model = SwinTransformer(img_size=224, num_classes=0,depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32],embed_dim=128,drop_path_rate=0.)
...
#     for block in list(model.children())[2][:2]:
#         for param in list(block.parameters()):
#             param.requires_grad = False
