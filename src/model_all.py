...
class MultiModal(nn.Module):
    def __init__(self, args, bert_cfg_dict, model_path, task=['tag', 'mlm', 'mfm'],model_cfg=MODEL_CONFIG, init_from_pretrain=True):
        super().__init__()
        ...
        self.roberta2 = ALBEF(text_encoder= BERT_PATH ,config = BERT_config, setup_mode="finetune",parm_cfg = model_cfg,args = args)
    ...
    def forward(self, inputs, inference=False,
        ...
        video_feature_raw = self.video_fc(video_feature)  # 这个fc有点关键
        ...
        features, mask, lm_prediction_scores, encoder_last4_output = self.roberta(video_feature_raw, video_mask,
...
#             mean_video_feature = torch.einsum("bsh,bs,b->bh", video_feature_tg, video_mask_tf.half(), 1 / video_mask_tf.half().sum(dim=1) + 1e-9)
            ...
            pred2 = self.newfc_tag2(torch.relu(in_features))  # 
            ...
            if self.args.onnx:
                ...
                return pred,pred2, torch.argmax(pred, dim=1), torch.argmax(pred2, dim=1)
            else:
